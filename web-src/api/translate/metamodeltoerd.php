...
/**
   ...
   @code
   curl -d 'json={\"classes\": [{\"attrs\":[], \"methods\":[], \"name\": \"Hi World\"}]}' http://host.com/api/translate/metamodeltoerd.php";
   @endcode
...
Use, for example:
    ...
    $metastrategy = new UMLMeta();
    $metastrategy->create_metamodel($_POST['json']);
    $eerstrategy = new Meta2EER();
    $eerstrategy->create_modelKF($metastrategy->get_json());
    $eer = $eerstrategy->get_json();
	print_r($eer);
}
